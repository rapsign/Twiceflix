...
const VolumeControl = ({ muted, volume, onMuteToggle, onVolumeChange }) => {
  ...
  return (
    ...
    >
      ...
      {/* Slider — muncul saat hover, satu bg dengan icon */}
      <div
        className="flex items-center pr-3 transition-all duration-200"
        style={{
          width: hovered ? "72px" : "0px",
          opacity: hovered ? 1 : 0,
        }}
      >
        <input
...
const ShortPlayer = ({
  ...
  // Sync volume state ke parent (untuk VolumeControl di luar player)
  useEffect(() => {
    onVolumeState?.({ muted, volume });
  }, [muted, volume]);
  ...
  return (
    <>
      ...
      >
        ...
        {/* Top-left: Pause + Volume — desktop only, mobile dihandle MobileShorts */}
        {isActive && !isMobile && (
          <div
...
const DesktopShorts = ({ getShort, total }) => {
  ...
  // Track active short untuk share/report di luar video
  useEffect(() => {
